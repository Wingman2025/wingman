...
# Endpoint temporal para deployment en Railway
@app.route('/deploy-companion', methods=['GET'])
def deploy_companion():
    """Endpoint temporal para aplicar migraciones y ejecutar seed en Railway"""
    try:
        # Detectar entorno
        ...
        result['steps'].append('🔄 Aplicando migraciones...')
        db.create_all()
        result['steps'].append('✅ Migraciones aplicadas')
        
        # Paso 2: Verificar estado actual
        from models import GoalTemplate, Badge
        existing_templates = GoalTemplate.query.count()
        existing_badges = Badge.query.count()
        result['steps'].append(f'📊 Estado actual: {existing_templates} plantillas, {existing_badges} badges')
        
        # Paso 3: Ejecutar seed si es necesario
        if existing_templates > 0 and existing_badges > 0:
            result['steps'].append('ℹ️  Los datos ya existen, no es necesario hacer seed')
            result['status'] = 'success'
            return jsonify(result)
        
        # Ejecutar seed
        result['steps'].append('🌱 Ejecutando seed de datos iniciales...')
        ...
        if templates_created > 0 or badges_created > 0:
            db.session.commit()
            result['steps'].append(f'✅ Seed completado: {templates_created} plantillas, {badges_created} badges creados')
        else:
            result['steps'].append('ℹ️  Todos los datos ya existían')
        
        # Estado final
        final_templates = GoalTemplate.query.count()
        final_badges = Badge.query.count()
        result['steps'].append(f'📊 Estado final: {final_templates} plantillas, {final_badges} badges')
